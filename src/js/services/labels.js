...
const labels = require('../models/labels'); // the mongoose users model
...
const initializeDefaultLabels = async () => {
  const defaultLabels = [
    ...
    const exists = await labels.findOne({ id: label.id });
    if (!exists) {
...
Returns all matching labels as an array */
const getLabels = async (field, value) => {  
  return await labels.find({[field]: value}); // returns an array of matching labels
};
...
* }
*/
const updateLabel = async (id, updatedLabel) => {
  const updated = await labels.findOneAndUpdate(
    { id },                    
    { $set: updatedLabel },    // apply updates
...
const deleteLabel = async (id) => {
  const deletedLabel = await labels.findOneAndDelete({ id });
 return deletedLabel;
}
...
* }
*/
const getDefaultLabelForUser = async() => {
return await labels.find({userId:'system'});
};
